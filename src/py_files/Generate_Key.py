...
class Generate_Key(QtWidgets.QDialog, Generate_Key_Design.Ui_Dialog):
    ...
    def generateKey(self):
        ...
        try:
            ...
            self.ssh.connect("127.0.0.1", 22, username=str(self.lineEditUsername.text()), password=str(self.lineEditPassword.text()), allow_agent=False, look_for_keys=False)
            ...
            #close the ssh connection in order to avoid having to much ssh to other guy !!!there is a limit in etc/ssh/sshd_config
            self.ssh.close()

            #reopen the ssh connection
            self.ssh = paramiko.SSHClient()
            self.ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            self.ssh.connect("127.0.0.1", 22, username=str(self.lineEditUsername.text()),
                             password=str(self.lineEditPassword.text()), allow_agent=False, look_for_keys=False)

            # launch the different thread which will generate the public keys
            ...
            temp = QtWidgets.QMessageBox.warning(self, "Warning", "Failed to login to the admin session, please reenter the admin username and the admin password")
    ...
    def launchKeyGenerationThread(self, index):
        ...
        if self.error[index] == False:
            self.outputString =  self.outputString +"V - "+ self.ipList[index] + ": has his rsa public key set up \n"
